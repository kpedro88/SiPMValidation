...
class DigiClass2 : public DigiClass {
	public:
		...
		pair<TH2F*,TGraphErrors*> Loop(int cut, bool rechit, string plotalldir){
...
void plotPulses(vector<string> fnames, vector<string> legnames, vector<string> outnames, vector<int> cuts, vector<Color_t> colors, bool norm=false, bool curve=false, bool rechit=false, bool plotall=false){
	vector<TFile*> files;
	...
	for(unsigned f = 0; f < fnames.size(); ++f){
		...
		pair<TH2F*,TGraphErrors*> res = dc->Loop(cuts[f],rechit,plotalldir);
